### model_train.py implementation for the 3rd Model ###
...
df= pd.read_csv("/subsets/train_set3.csv")
df.sort_values('index', axis = 0, inplace=True)
df = df.set_index('index')
label_df = df.pop('class')

# Define the XGBoost classifer for the 3rd model
clf = XGBClassifier(max_depth=9, learning_rate=0.1, n_estimators=1500, silent=True, objective='binary:logistic', nthread=-1, gamma=0.2, min_child_weight=12, max_delta_step=0, subsample=0.5, colsample_bytree = 0.8)
clf.fit(df, label_df)
...
joblib.dump(clf, '/trained_models/model_3.pkl', compress=0)
