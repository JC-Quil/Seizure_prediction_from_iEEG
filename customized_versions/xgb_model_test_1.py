### model_test.py implmentation for the 1st model ###
...
df = pd.read_csv("subsets/test_set1.csv")
...
clf = joblib.load('trained_models/model_1.pkl')
...
dic_final = {'File' : [],
            'Class predict1' : np.zeros(train_data_labels_safe.shape[0]),
            'Proba1' : np.zeros(train_data_labels_safe.shape[0]), 
...
for i in range(train_data_labels_safe.shape[0]):
    ...
    if ((i*16*20+1) >= y_pred_df['ind'].iloc[0]) & ((i*16*20+1) <= y_pred_df['ind'].iloc[-1]):
        ...
        if not pred_final.empty: 
            ...
            dic_final['Class predict1'][i] = pred_final
            dic_final['Proba1'][i] = proba_final 


# Record the predictions dataframe in a .csv file
file_name = "predictions/predictions_model1.csv"
df_final = pd.DataFrame(dic_final, columns = ['File','Class predict1', 'Proba1', 'Class'], index = range(train_data_labels_safe.shape[0]))
df_final = df_final.set_index(['File'])
df_final = df_final.loc[df_final['Class predict1'] != -1]
df_final = df_final.dropna(subset = ['Class predict1'])
df_final.to_csv(file_name)

# Print metrics evaluations
print "F1 score for test set: {:.4f}.".format(f1_score(df_final['Class'], df_final['Class predict1'], pos_label=1.0))
print "Recall score for test set: {:.4f}.".format(recall_score(df_final['Class'], df_final['Class predict1'], pos_label=1.0, average='binary'))
print "ROC score for test set: {:.4f}.".format(roc_auc_score(df_final['Class'], df_final['Class predict1'], average = 'macro'))
print "ROC score proba for test set: {:.4f}.".format(roc_auc_score(df_final['Class'], df_final['Proba1'], average = 'macro'))
